...
First load the libraries and the PBMC dataset
```{r}
library(dplyr)
library(Seurat)
library(patchwork)
...
```
The output is a sparse-matrix where "." represent 0. Since most values in scRNA-Seq matrix are 0, Seurat uses a sparse-matrix representation whenever possible. This results in significant memory and speed savings for Drop-Seq/inDrop/10x data.
```{r}
#Let's see the size of a dense matrix vs a sparse-matrix
...
* We filter cells that have unique feature counts over 2500 or less than 200
...
n Seurat v2 we also use the ScaleData() function to remove unwanted sources of variation from a single-cell dataset. For example, we could ‘regress out’ heterogeneity associated with (for example) cell cycle stage, or mitochondrial contamination. These features are still supported in ScaleData() in Seurat v3, i.e.:
```{r}
...
```
DoHeatMap() generates an expression heatmap for given cells and features. In this case, we are plotting the top 20 markers (or al markers if less than 20) for each cluster.
```{r}
pbmc.markers %>%
  group_by(cluster) %>%
